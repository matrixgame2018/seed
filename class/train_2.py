...
from media_stom import dataset
import numpy as np
from media_stom.args import args
from media_stom.build_net import make_model
from media_stom.transform import get_transforms
from media_stom.utils import Bar, Logger, AverageMeter, accuracy, mkdir_p, savefig, get_optimizer, save_checkpoint
...
def main():
    ...
    data_dir = 'D:/weiai_dataset/train/class/'
    train_folder = os.path.join(data_dir, "train")
    val_folder = os.path.join(data_dir, "train")
    trainset = datasets.ImageFolder(train_folder, transform=transform['val_train'])
    train_loader = data.DataLoader(trainset, batch_size=args.train_batch, shuffle=True, num_workers=args.workers, pin_memory=True)

    valset = datasets.ImageFolder(val_folder, transform=transform['val_test'])
